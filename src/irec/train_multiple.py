...
from irec.train import train
from irec.infer import inference

from irec.callbacks import BaseCallback, EvalCallback, ValidationCallback
...
def main():
    ...
    for (
        ...
        loss_param,
        optimizer_param,
    ) in list_of_params[start_from:num]:
        cnt += 1
        if cnt < start_from:
            continue
        ...
        train_sampler, validation_sampler, eval_sampler = (
        ...
        if irec.utils.tensorboards.GLOBAL_TENSORBOARD_WRITER is not None:
            irec.utils.tensorboards.GLOBAL_TENSORBOARD_WRITER.close()
        irec.utils.tensorboards.GLOBAL_TENSORBOARD_WRITER = (
            irec.utils.tensorboards.TensorboardWriter(
                model_name,
                use_time=False,
            )
        )

        model = BaseModel.create_from_config(model_param, **dataset.meta).to(
            DEVICE,
        )
        ...
        eval_model = BaseModel.create_from_config(
            model_param,
            **dataset.meta,
        ).to(DEVICE)
